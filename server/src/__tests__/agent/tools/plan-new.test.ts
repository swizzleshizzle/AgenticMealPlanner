...
beforeEach(async () => {
  ...
  await prisma.weeklyPlan.deleteMany({ where: { weekStartDate: { in: [new Date("2026-05-11"), new Date("2026-05-25")] } } });
...
describe("add_planned_meal", () => {
  it("creates a PlannedMeal on an existing plan", async () => {
    ...
    await prisma.weeklyPlan.create({ data: { weekStartDate: new Date("2026-05-11"), status: "active" } });
    const result: any = await addPlanned.handler(
      { weekStartDate: "2026-05-11", mealId: meal.id, day: "wednesday", mealSlot: "dinner", servings: 4 },
      ctx,
    );
  ...
  it("creates a new WeeklyPlan if one doesn't exist for the week", async () => {
    ...
    const result: any = await addPlanned.handler(
      { weekStartDate: "2026-05-25", mealId: meal.id, day: "friday", mealSlot: "dinner", servings: 2 },
      ctx,
    );
    ...
    expect(plan?.weekStartDate.toISOString().slice(0, 10)).toBe("2026-05-25");
  });
});
